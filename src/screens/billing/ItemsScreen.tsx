...
import { matchesWords, sameText, searchHaystack, searchWords } from '../../utils/search';

/** Your price list: every item with its fixed price and how many are left. */
export const ItemsScreen: React.FC = () => {
  const { products, deleteProduct, can, godowns, stockBatches, schemes, purchases, purchaseOrders, isFieldVisible } = useTrading();
  // Cost prices and the stock value (at cost) are hidden from roles that may not see purchase costs (operator, viewer).
  const showCosts = isFieldVisible('purchase_costs');
  const canAddItem = can('products:create');
  const canEditItem = can('products:create') || can('edit_prices');
  ...
  const rows = useMemo(() => {
    // Name, code, barcode (whole or part), group and brand; every word in any order; Urdu keyboards too.
    const words = searchWords(query);
    return products
      .filter((p) => matchesWords(words, searchHaystack([p.name, p.code, p.barcode, itemGroup(p), p.brand])))
      .filter((p) => (!group || sameText(itemGroup(p), group)) && (!brand || sameText(p.brand, brand)))
  ...
  const actions = (p: Product) => (
    <>
      ...
      {canEditItem && <RowAction label={`Edit ${p.name}`} text="Edit" alwaysText icon={<Pencil className="w-4 h-4" />} onClick={() => ui.editItem(p.id)} />}
  ...
  return (
    <div className="space-y-5">
      <PageHeader title="Items & Prices" subtitle={<>{products.length} item{products.length === 1 ? '' : 's'}{showCosts && stockValue > 0 ? <> • stock worth <span className={moneyCls}>{rs(stockValue)}</span></> : ''}{lowCount > 0 ? ` • ${lowCount} low` : ''}</>}>
        ...
        {canAddItem && <button type="button" onClick={() => ui.newItem()} className={primaryBtn}><Plus className="w-4 h-4 text-teal-400 dark:text-teal-700" /> New item</button>}
      </PageHeader>
      <div className="flex gap-2">
        <div className="relative flex-1">
          <Search className="w-4 h-4 absolute left-3.5 top-1/2 -translate-y-1/2 text-[#9CA3AF]" />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search name, code, barcode, group or brand" className={`${inputCls} pl-10`} aria-label="Search items" />
        </div>
 ...
 text={query || group || brand ? 'No item matches that search.' : 'No items yet. Add the things you sell with their prices.'}
            action={!query && !group && !brand && canAddItem && <button type="button" onClick={() => ui.newItem()} className={secondaryBtn}><Plus className="w-4 h-4" /> New item</button>}
          />
        </div>
      ) : (
        <>
                ...
                <li key={p.id} className="px-4 py-3">
                  <div className="flex items-start gap-3">
                    ...
                    <button type="button" onClick={() => stock.itemHistory(p.id)} className="flex-1 min-w-0 text-left" aria-label={`History of ${p.name}`} title="Stock history">
                      ...
                      <span className="block text-[11px] text-[#6B7280] dark:text-[#8E9299]">{meta(p) ? `${meta(p)} • ` : ''}per {p.unit || 'pcs'}{hasPack(p) ? ` • 1 ${p.packName} = ${p.packSize} ${p.unit || 'pcs'}` : ''}{showCosts && p.costPricePerKg ? ` • cost ${rs(p.costPricePerKg)}` : ''}{p.trackBatches ? ' • batch & expiry' : ''}</span>
                    </button>
                  ...
                  <div className="mt-2 flex items-center gap-2 flex-wrap text-xs">
                    ...
                    {showCosts && <span className="text-[#6B7280] dark:text-[#94A3B8]">worth <span className={`${moneyCls} font-semibold text-[#374151] dark:text-[#CBD5E1]`}>{rs(valueOf(p))}</span></span>}
                  </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr>
                  ...
                  {showCosts && <th scope="col" className={`${thCls} text-right`} title="Stock × cost price (sale price when no cost is set)">Stock value</th>}
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-[#FAF9F6] dark:hover:bg-[#162436] transition-colors align-top">
                      <td className="px-4 py-3">
                        ...
                        <div className="text-[11px] text-[#6B7280] dark:text-[#8E9299]">{meta(p) ? `${meta(p)} • ` : ''}per {p.unit || 'pcs'}{hasPack(p) ? ` • 1 ${p.packName} = ${p.packSize} ${p.unit || 'pcs'}` : ''}{showCosts && p.costPricePerKg ? ` • cost ${rs(p.costPricePerKg)}` : ''}{p.trackBatches ? ' • batch & expiry' : ''}</div>
                        <ItemStockDetails product={p} />
                        </div></div>
                      </td>
                      ...
                      {showCosts && <td className={`px-4 py-3 text-right text-[#374151] dark:text-[#CBD5E1] ${moneyCls}`}>{rs(valueOf(p))}</td>}
