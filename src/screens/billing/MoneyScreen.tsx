      ...
      {tab === 'cashbook' && !book && (
        <div className={`${cardCls} overflow-hidden`}>
          ...
          {monthMoves.length === 0 ? <EmptyState compact icon={<Coins className="w-5 h-5" />} text="No money moved this month." /> : (
            <ul className="divide-y divide-[#F1F0EC] dark:divide-[#1E2E40]">
              {monthMoves.map((m) => (
                <li key={m.id} className="flex items-center gap-2 px-4 sm:px-5 py-2.5 text-sm hover:bg-[#FAF9F6] dark:hover:bg-[#162436] transition-colors"><span className="text-xs text-[#6B7280] dark:text-[#8E9299] w-20 shrink-0 tabular-nums">{formatDate(m.date)}</span>{m.reference && <span className="tabular-nums text-xs font-bold text-[#111827] dark:text-white shrink-0" data-testid="cashbook-ref" title="Receipt / voucher no.">{m.reference}</span>}<span className="flex-1 min-w-0 truncate text-[#374151] dark:text-[#CBD5E1]">{m.counterparty ? `${m.counterparty} • ` : ''}{m.description}<span className="text-[11px] text-[#6B7280] dark:text-[#8E9299]"> • {m.method || 'Cash'}{m.bankCode && bankAccounts.length > 1 ? ` • ${bankName(m.bankCode) || m.bankCode}` : ''}</span></span><span className={`tabular-nums whitespace-nowrap font-bold ${m.direction === 'in' ? 'text-teal-700 dark:text-teal-300' : 'text-rose-700 dark:text-rose-300'}`}>{m.direction === 'in' ? '+' : '−'} {rs(m.amount)}</span><EditPaymentButton row={ledgerById.get(m.sourceId)} /></li>
              ))}
            </ul>
          )}
        </div>
          ...
          <ul className="divide-y divide-[#F1F0EC] dark:divide-[#1E2E40]">
            ...
            {book.rows.map(({ m, balance }) => (
              <li key={m.id} className="flex flex-wrap sm:flex-nowrap items-center gap-x-2 px-4 sm:px-5 py-2.5 text-sm hover:bg-[#FAF9F6] dark:hover:bg-[#162436] transition-colors" data-testid="book-row">
                ...
                {m.reference && <span className="tabular-nums text-xs font-bold text-[#111827] dark:text-white shrink-0" data-testid="cashbook-ref" title="Receipt / voucher no.">{m.reference}</span>}
