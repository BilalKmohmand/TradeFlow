      ...
      <div className="border-t border-[#E5E5E1] dark:border-[#203248] p-3 space-y-2">
        ...
        {(can('admin_screen') || can('system:admin_screen')) && (
          <button
            ...
            aria-label={isBilling ? 'Full trading suite' : 'Back to simple billing'}
            className={`w-full flex items-center gap-2 rounded-2xl px-3 py-2 text-[11px] font-semibold bg-indigo-50 dark:bg-indigo-950/40 text-indigo-800 dark:text-indigo-300 hover:bg-indigo-100 dark:hover:bg-indigo-950/70 ${collapsed ? 'justify-center' : ''}`}
          >
            <ArrowLeftRight className="w-3.5 h-3.5 shrink-0" />
            {!collapsed && <span className="truncate">{isBilling ? 'Full trading suite' : 'Back to simple billing'}</span>}
          </button>
        )}
        {!collapsed && (
