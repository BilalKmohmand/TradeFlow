...
export const PrintDocument: React.FC<PrintDocumentProps> = ({ request, onClose }) => {
  ...
  const content = useMemo(() => {
        ...
        return {
          ...
          body: (
            <ThermalReceipt company={COMPANY} title={`INVOICE #${inv.invoiceNumber.replace(/^INV-/, '')}`} date={`${formatDate(inv.issueDate)}${time ? ` ${time}` : ''}`} footer={settings.billFooter}>
              ...
              {(inv.discount || 0) > 0 && <ThermalRow left="Carriage expenses" right={`-${money(inv.discount || 0)}`} />}
          ...
          <>
            ...
            <table className={`w-full text-xs ${paper === 'a5' ? 'mt-4' : 'mt-6'} border-collapse`}>
              ...
              <tfoot>
                ...
                {(inv.discount || 0) > 0 && <tr><td colSpan={span} className="pt-1 text-right text-[11px] text-gray-600">Carriage expenses</td><td className="pt-1 text-right font-mono px-3">− {money(inv.discount || 0)}</td></tr>}
    ...
    return null;
  }, [request, paper, books, billingReport, salesExtrasReport, purchasingDoc, financeDoc, classicDoc, booksDoc, salesmen, areas, dispatches, bookings, customers, suppliers, products, ledger, trucks, settings, quotations, purchaseOrders, returns, invoices, expenses, cashEntries, bankStatementLines, bankReconciliations, cheques]);
