...
export const PrintDocument: React.FC<PrintDocumentProps> = ({ request, onClose }) => {
  ...
  const content = useMemo(() => {
    ...
    return null;
  }, [request, books, dispatches, bookings, customers, suppliers, products, ledger, trucks, settings, quotations, purchaseOrders, returns, invoices, expenses, cashEntries]);
          ...
          {content ? (
            <>
              ...
              <div className="mt-10 pt-3 border-t border-gray-200 flex items-center justify-between text-[10px] text-gray-500">
                ...
                <span>{request.type === 'bill' || request.type === 'daily_sheet' || isAccountingPrint(request) ? 'All amounts in PKR (Rs.)' : 'All quantities in kg • amounts in PKR'}</span>
              </div>
            </>
          ) : (
