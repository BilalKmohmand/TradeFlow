...
export const Modal: React.FC<{ isOpen: boolean; onClose: () => void; title: string; subtitle?: string; /** true: a wider sheet; 'xl': wider still on a desktop (bill / invoice lines). */ wide?: boolean | 'xl'; children: React.ReactNode; footer?: React.ReactNode }> = ({ isOpen, onClose, title, subtitle, wide, children, footer }) => {
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4">
          ...
          <motion.div
            ...
            className={`relative z-10 w-full ${wide === 'xl' ? 'sm:max-w-3xl lg:max-w-5xl' : wide ? 'sm:max-w-3xl' : 'sm:max-w-lg'} max-h-[92dvh] sm:max-h-[90vh] flex flex-col bg-white dark:bg-[#101A26] rounded-t-[28px] sm:rounded-[28px] border border-b-0 sm:border-b border-[#E5E5E1] dark:border-[#203248] shadow-2xl`}
          >
