...
export const formatCell = (col: Pick<ReportColumn, 'money' | 'key'>, v: Cell): string => {
  if (v == null || v === '') return '';
  if (typeof v === 'number') return col.money ? MONEY.format(Math.round(v * 100) / 100) : QTY.format(v);
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        {report.summary.map((s) => (
          <div key={s.label} className="rounded-2xl bg-white dark:bg-[#101A26] border border-[#E5E5E1] dark:border-[#203248] px-3.5 py-2.5">
            ...
            <div className="text-base font-extrabold tabular-nums text-[#111827] dark:text-white break-words" data-testid={`summary-${s.label}`}>{s.money && typeof s.value === 'number' ? `Rs. ${MONEY.format(s.value)}` : String(s.value ?? '')}</div>
          </div>
        ))}
      </div>
    )}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm" data-testid="report-table">
            <thead className="bg-[#FAF9F6] dark:bg-[#162436]">
              <tr>
                {s.columns.map((c) => <th key={c.key} className={`px-3 py-2 text-[10px] font-bold uppercase tracking-wider text-[#6B7280] dark:text-[#94A3B8] whitespace-nowrap ${c.align === 'right' ? 'text-right' : 'text-left'}`}>{c.label}</th>)}
            ...
            <tbody className="divide-y divide-[#F1F0EC] dark:divide-[#1E2E40]">
              ...
              {s.rows.map((r, ri) => (
                <tr key={ri} className={`${rowCls(r)} ${r.billId && onOpenBill ? 'cursor-pointer hover:bg-[#FAF9F6] dark:hover:bg-[#162436]' : ''}`} onClick={r.billId && onOpenBill ? () => onOpenBill(r.billId!) : undefined}>
                  {s.columns.map((c) => <td key={c.key} className={`px-3 py-1.5 ${c.align === 'right' ? 'text-right tabular-nums whitespace-nowrap' : ''} ${r.style === 'heading' && c.key !== s.columns[0].key ? '' : ''}`}>{formatCell(c, r.cells[c.key])}</td>)}
  ...
  <div className="space-y-3">
    {report.summary && report.summary.length > 0 && (
      <div className="text-[10.5px] flex flex-wrap gap-x-5 gap-y-0.5 justify-center">
        {report.summary.map((s) => <span key={s.label}>{s.label}: <b>{s.money && typeof s.value === 'number' ? `Rs. ${MONEY.format(s.value)}` : String(s.value ?? '')}</b></span>)}
      </div>
    )}
    {report.sections.map((s, si) => (
      <div key={si}>
          ...
          <tbody>
            ...
            {s.rows.map((r, ri) => (
              <tr key={ri} className={r.style === 'heading' ? 'bg-gray-100 font-bold' : r.style === 'subtotal' || r.style === 'total' ? 'font-bold' : ''}>
                {s.columns.map((c) => <td key={c.key} className={`${ptd} ${c.align === 'right' ? 'text-right font-mono whitespace-nowrap' : ''}`}>{formatCell(c, r.cells[c.key])}</td>)}
              </tr>
            ))}
          </tbody>
          {s.totals && s.rows.length > 0 && (
            <tfoot>
              <tr className="font-extrabold">{s.columns.map((c) => <td key={c.key} className={`border-2 border-gray-900 px-1.5 py-1 text-[10.5px] ${c.align === 'right' ? 'text-right font-mono whitespace-nowrap' : ''}`}>{formatCell(c, s.totals![c.key] ?? null)}</td>)}</tr>
            </tfoot>
          )}
        </table>
      </div>
