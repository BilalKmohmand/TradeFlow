    ...
    <div>
            ...
            <tbody className="divide-y divide-[#F1F0EC] dark:divide-[#1E2E40]" data-testid="customer-rates">
              {rates.map(({ r, p }) => (
                <tr key={r.id}>
                  ...
                  <td className="px-3 py-2 text-right font-mono font-bold">{rs(r.agreedRatePerKg)}<span className="text-[11px] font-sans font-normal text-[#8E9299]">/{p!.unit || 'pcs'}</span></td>
                  <td className="px-3 py-2 text-right font-mono text-[#8E9299] hidden sm:table-cell">{rs(p!.unitPricePerKg)}</td>
                  {canEdit && (
      ...
      {canEdit && (
        <form onSubmit={save} className="grid grid-cols-12 gap-2 items-center">
          ...
          <input aria-label="Their rate" type="number" inputMode="decimal" min="0" step="any" value={rate} onChange={(e) => setRate(e.target.value)} placeholder="Their rate" className={`${inputCls} font-mono col-span-7 sm:col-span-3`} />
