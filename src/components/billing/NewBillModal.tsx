      ...
      <div className="space-y-5" ref={box} onKeyDown={onKeys}>
          ...
          {selected && snapshot && (
            <div data-testid="bill-customer-info" className="sm:col-span-3 rounded-2xl border border-[#E5E5E1] dark:border-[#203248] bg-[#FAF9F6] dark:bg-[#162436] px-3.5 py-2.5 text-xs text-[#374151] dark:text-[#CBD5E1] space-y-1">
              <div className="flex flex-wrap gap-x-4 gap-y-1">
                <span>Balance <strong className={`font-mono ${selected.totalDue > 0 ? 'text-amber-700 dark:text-amber-300' : selected.totalDue < 0 ? 'text-teal-700 dark:text-teal-300' : ''}`}>{rs(selected.totalDue)}</strong>{selected.totalDue < 0 ? ' (advance)' : ''}</span>
                <span>Credit left <strong className="font-mono">{credit.hasLimit ? rs(credit.available) : 'no limit'}</strong></span>
                <span>Last payment {snapshot.lastPayment ? <strong className="font-mono">{rs(snapshot.lastPayment.amount)}</strong> : <strong>none</strong>}{snapshot.lastPayment ? ` on ${formatDate(snapshot.lastPayment.date)}${snapshot.lastPayment.method ? ` (${snapshot.lastPayment.method})` : ''}` : ''}</span>
              </div>
              {snapshot.lastBills.length > 0 && (
                <div className="flex flex-wrap gap-x-3 gap-y-0.5 text-[11px] text-[#6B7280] dark:text-[#94A3B8]">
                  ...
                  {snapshot.lastBills.map((b) => (
                    <span key={b.id} className="whitespace-nowrap">{b.invoiceNumber} • {formatDate(b.issueDate)} • <span className="font-mono">{rs(b.totalAmount)}</span>{b.balanceDue > 0 ? <span className="text-amber-700 dark:text-amber-300"> ({rs(b.balanceDue)} due)</span> : ''}</span>
                  ))}
                </div>
              )}
            </div>
              ...
              return (
                <div key={l.key} data-row={l.key} className="grid grid-cols-12 gap-2 items-center rounded-2xl border border-[#E5E5E1] dark:border-[#203248] p-2 sm:p-1 sm:border-0">
                  ...
                  <div className="col-span-4 sm:col-span-2">
                    ...
                    <input aria-label={`Quantity ${idx + 1}`} data-nav="qty" type="number" inputMode="decimal" min="0" step="any" value={l.rawQty} onChange={(e) => setRow(l.key, { qty: e.target.value })} className={`${inputCls} font-mono`} placeholder="Qty" />
                  </div>
                  <div className="col-span-4 sm:col-span-2">
                    <span className={smallLabel}>Price{p ? ` per ${unitWord}` : ''}</span>
                    <input aria-label={`Price ${idx + 1}`} data-nav="price" type="number" inputMode="decimal" min="0" step="any" value={l.rawPrice} onChange={(e) => setRow(l.key, { price: e.target.value, priceFrom: 'typed' })} className={`${inputCls} font-mono`} placeholder="Price" />
                  ...
                  <div className="col-span-3 sm:col-span-2 text-right font-mono font-bold text-sm text-[#111827] dark:text-white"><span className={`${smallLabel} font-sans`}>Amount</span>{rs(l.amount)}</div>
                  ...
                  {p && (
                    <div data-testid={`line-info-${idx + 1}`} className="col-span-12 flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-[#6B7280] dark:text-[#94A3B8] px-1">
                      ...
                      <span>Stock <strong className={`font-mono ${p.stockKg < 0 ? 'text-rose-700 dark:text-rose-300' : 'text-[#374151] dark:text-[#CBD5E1]'}`}>{formatPackQty(p.stockKg, p, 'short')}</strong></span>
                      {last && (
                        <button type="button" tabIndex={-1} onClick={() => setRow(l.key, { price: shownPrice(p.id, last.rate, l.inPack), priceFrom: 'typed' })} className="hover:text-teal-700 dark:hover:text-teal-300" title="Use this rate">
                          Last rate <strong className="font-mono text-[#374151] dark:text-[#CBD5E1]">{rs(last.rate)}/{p.unit || 'pcs'}</strong> ({formatDate(last.date)})
                        </button>
                      )}
                    </div>
                  )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="rounded-2xl bg-[#FAF9F6] dark:bg-[#162436] border border-[#E5E5E1] dark:border-[#203248] p-4 space-y-2 text-sm">
              ...
              <div className="space-y-2 pt-1" data-testid="bill-split">
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <input id="bill-split-cash" data-nav="paid" type="number" inputMode="decimal" min="0" step="any" value={splitCash} onChange={(e) => setSplitCash(e.target.value)} className={`${inputCls} font-mono`} placeholder="0" />
                  </div>
                  <div>
                    <label className={labelCls} htmlFor="bill-split-cheque">Cheque</label>
                    <input id="bill-split-cheque" data-nav="split-cheque" type="number" inputMode="decimal" min="0" step="any" value={splitCheque} onChange={(e) => setSplitCheque(e.target.value)} className={`${inputCls} font-mono`} placeholder="0" />
                  </div>
                  <div>
                    <label className={labelCls} htmlFor="bill-split-bank">Bank / wallet</label>
                    <input id="bill-split-bank" data-nav="split-bank" type="number" inputMode="decimal" min="0" step="any" value={splitBank} onChange={(e) => setSplitBank(e.target.value)} className={`${inputCls} font-mono`} placeholder="0" />
                  </div>
                  <div>
                ...
                <div className="grid grid-cols-2 gap-2 pt-1">
                  <div>
                    ...
                    <div className="flex gap-1">
                      <input id="bill-paid" data-nav="paid" type="number" inputMode="decimal" min="0" step="any" value={paidNow} onChange={(e) => setPaidNow(e.target.value)} className={`${inputCls} font-mono`} placeholder="0" />
            ...
            {split && payment.paid > 0 && !payment.error && <div className="flex justify-between text-xs text-[#6B7280] dark:text-[#94A3B8]"><span>Paid now</span><span className="font-mono">{rs(payment.paid)}</span></div>}
