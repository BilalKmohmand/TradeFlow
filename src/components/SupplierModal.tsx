...
const SupplierForm: React.FC<{
  ...
  const [code, setCode] = useState(editing ? editing.code || '' : nextPartyCode(suppliers, SUPPLIER_CODE_PREFIX));
  ...
  const [opening, setOpening] = useOpeningBalance('supplier', editing?.id);
  ...
  const submit = (e: React.FormEvent) => {
    ...
    const saved = editing ? { id: partyId } : addSupplier(data);
    if (ob.amount !== opening.was || (ob.amount !== 0 && opening.date !== opening.wasDate)) {
      const r = setOpeningBalance('supplier', saved.id, ob.amount, opening.date);
      if (!r.success) {
        busy.current = false;
  ...
  return (
    <form onSubmit={submit} className="space-y-4" aria-label={editing ? 'Edit supplier' : 'New supplier'}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          <label className={labelCls} htmlFor="sup-code">Supplier ID</label>
          <input id="sup-code" value={code} onChange={(e) => setCode(e.target.value)} className={`${inputCls} tabular-nums`} placeholder="Given automatically" autoCapitalize="characters" />
        </div>
        <div>
        ...
        <OpeningBalanceFields idPrefix="sup" entityType="supplier" value={opening} onChange={setOpening} />
      </div>
