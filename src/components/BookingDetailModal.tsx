    ...
    <>
                    ...
                    <table className="w-full text-left text-xs">
                      ...
                      <tbody className="divide-y divide-[#FAF9F6] font-mono">
                        {bookingDispatches.length === 0 ? (
                          <tr><td colSpan={9} className="py-8 text-center text-[#8E9299] font-sans">No dispatches yet.</td></tr>
                        ) : (
                              ...
                              <td className="py-2.5 px-4 font-sans whitespace-nowrap">
                                {(d.status ?? 'in_transit') === 'delivered' ? (
                                  <span className="inline-flex flex-col">
                                    <span className="text-[10px] font-bold text-teal-800 bg-teal-50 border border-teal-200 px-2 py-0.5 rounded-full w-fit">Delivered</span>
                                    <span className="text-[10px] text-[#8E9299] mt-0.5">{d.deliveredAt ? formatDate(d.deliveredAt) : ''}{d.receivedBy ? ` • ${d.receivedBy}` : ''}</span>
      ...
      {deliverId && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
          ...
          >
            <div>
              <h3 className="text-base font-bold text-[#111827]">Confirm delivery</h3>
              <p className="text-xs text-[#6B7280] mt-1">Proof of delivery for {bookingDispatches.find((d) => d.id === deliverId)?.dispatchNumber}. The vehicle becomes available again.</p>
            </div>
            <div>
              <label className="block text-[10px] font-bold text-[#8E9299] mb-1.5 uppercase tracking-widest">Received by</label>
              <input autoFocus value={receivedBy} onChange={(e) => setReceivedBy(e.target.value)} placeholder="Name at the customer site" className="w-full bg-[#FAF9F6] border border-[#E5E5E1] rounded-2xl px-3.5 py-2.5 text-xs text-[#111827] focus:outline-hidden focus:border-teal-600" />
            </div>
            <div>
              <label className="block text-[10px] font-bold text-[#8E9299] mb-1.5 uppercase tracking-widest">POD note</label>
              <input value={podNote} onChange={(e) => setPodNote(e.target.value)} placeholder="Challan signed, shortage, remarks..." className="w-full bg-[#FAF9F6] border border-[#E5E5E1] rounded-2xl px-3.5 py-2.5 text-xs text-[#111827] focus:outline-hidden focus:border-teal-600" />
            </div>
            <div className="flex justify-end gap-2">
              <button type="button" onClick={() => setDeliverId(null)} className="px-4 py-2.5 rounded-2xl text-xs font-semibold text-[#6B7280] hover:bg-[#FAF9F6]">Cancel</button>
              <button type="submit" className="px-5 py-2.5 rounded-2xl bg-teal-700 hover:bg-teal-800 text-white text-xs font-bold">Mark delivered</button>
            </div>
          </form>
        </div>
      )}
