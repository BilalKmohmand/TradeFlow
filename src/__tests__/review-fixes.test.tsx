...
describe('review finding 2: cost of goods on bills', () => {
  it('posts cost of goods from the purchase cost when the item has no cost price', async () => {
  ...
  it('uses the cost of the batches actually sold', async () => {
    const { result } = await setup({ trackBatches: true });
...
describe('review finding 3a: stock that arrives with no supplier bill', () => {
  it('is kept as a stock record posted to Suspense, not hidden in opening equity', async () => {
    const { result } = await setup({ productCost: 30 });
  ...
  it('a stock figure changed on the item form becomes a stock-count record', async () => {
    const { result } = await setup({ productCost: 30 });
...
describe('review finding 3b: advances are kept, not turned into equity', () => {
  it('overpaying leaves a negative balance that shows as money the shop owes back', async () => {
    const { result } = await setup();
...
describe('review finding 5 and 6: godowns and purchases', () => {
  it('a plain item is not oversold from the main godown by eating another godown\'s stock', async () => {
    const { result } = await setup();
    let g2 = '';
  ...
  it('deleting a purchase removes the batch that purchase created', async () => {
    const { result } = await setup({ trackBatches: true });
  ...
  it('stock returned to a deleted godown goes back to the main godown', async () => {
    const { result } = await setup();
    let g2 = '';
    act(() => { g2 = result.current.addGodown('Godown B').godown!.id; });
    act(() => { result.current.receiveStock({ productId: 'p1', qty: 10, godownId: g2, costPrice: 50, supplierId: 's1' }); });
  ...
  it('a back-dated bill cannot sell a batch that has already expired', async () => {
    const { result } = await setup({ trackBatches: true });
...
describe('review finding 7: bank receipts', () => {
  it('a statement receipt can be recorded as a customer payment, reducing what they owe', async () => {
    const { result } = await setup();
  ...
  it('an unassigned receipt goes to Suspense even when the narration says "capital"', async () => {
    const { result } = await setup();
...
describe('review finding 9: the period lock covers everything dated', () => {
  it('bills, payments, deletions, transfers and bank entries in a closed period are refused', async () => {
    const { result } = await setup();
    let bill: any;
...
describe('review finding 11: permissions', () => {
  it('an operator cannot post journals, add accounts or change the bank reconciliation', async () => {
    const { result } = await setup();
...
describe('review finding 12 and low items', () => {
  it('loading the sample data clears journals, bank lines, batches and bills too', async () => {
    const { result } = await setup({ trackBatches: true });
  ...
  it('journal numbers never repeat after a deletion', async () => {
    const { result } = await setup();
...
describe('QA round on the ERP merge', () => {
  it('a journal with one account on both sides, or a future date, is refused', async () => {
    const { result } = await setup();
    let r: any;
  ...
  it('an operator cannot receive stock, move stock or manage godowns', async () => {
    const { result } = await setup();
    act(() => { result.current.logout(); });
    await signIn(() => result.current, OPERATOR);
    let r: any;
