...
describe('creditCheck (pure)', () => {
  ...
  it('flags when owes + unpaid part goes over, and reports what is available', () => {
  ...
  it('a fully paid bill never trips the limit, even for a customer already over', () => {
    expect(creditCheck({ totalDue: 15000, creditLimit: 10000 }, 0).over).toBe(false);
  });
  it('usage and over-limit list', () => {
...
describe('createBill credit limit', () => {
  it('blocks a bill that takes the customer over the limit', () => {
    const { result } = setup();
  ...
  it('allows a bill that stays under the limit', () => {
    const { result } = setup();
    let r: ReturnType<typeof result.current.createBill> | undefined;
  ...
  it('money paid now reduces the credit used', () => {
    const { result } = setup();
    let r: ReturnType<typeof result.current.createBill> | undefined;
  ...
  it('override works for a user with permission, needs a reason, and records it on the bill and in the audit log', () => {
    ...
    expect(inv.creditOverride).toMatchObject({ by: 'Administrator', reason: 'Pays every Friday', limit: 10000, dueAfter: 11000 });
  ...
  it('override is refused for a user without the override_credit permission', () => {
    const { result } = setup('operator');
