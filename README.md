...
## Apna Accountant layer (classic menu, invoices, reports)

The shop is moving from **Apna Accountant SB** (AG Soft). Everything staff had there is here, under the same names.

- **Classic menu** — Home shows the old program's 16 big buttons on a desktop (on a phone they are a compact grid under **More**): Product Coding → Items & Prices; Sale Invoice → New Bill; Purchase Invoice → the purchase invoice form; Product List → Product List / Rate List report; Daily Gross Profit, Daily Sale, Daily Purchase, Stock In Hand → those reports; Accounts Coding → chart of accounts; Account Ledger → account ledger; Cheque Deposits Bank → Money → Cheques; Books → cash book, bank book, day book, journal book, book balances; Vouchers → vouchers; Trial Balances, Profit & Loss, Balance Sheet → those reports. Turn it off in **Admin → System & Backups → Bills & stock → Show the classic menu** (on by default).
- **Sale Invoice** (New Bill) adds the old fields without changing the old ones: **Computer #** (the next bill number, read-only; it is given when you save), computer date, **Memo No** (your book / reference number, printed and searchable on Bills), **Delivery Order**, a **Stock in hand** box for the line being typed (with godowns and the party balance), and **Search bill #** to open an old bill by number. The bill shows *Bill date (your date)* and *entered on* (the time it was typed).
- **Delivery orders** — stock is taken when the bill is made, as in the old program; the bill then waits in **Reports → Pending Delivery List Report** (filters, print, CSV) until someone taps **Mark delivered** (date, who delivered, vehicle), which can print the delivery challan with the driver and vehicle filled in. A delivered bill can be set back to pending.
- **Purchase Invoice** (Purchase invoices screen, or the classic button): Computer # (series `P-`), date, supplier, **Memo No** (the supplier's bill no.), store / godown, lines with Code, Product, Unit, Packing (cartons), Qty, Rate, Amount; **Discount %** or amount, **Other charges**, Remarks, **Paid now** (cash / bank), print A4 / A5, a list with search by our number or the supplier's bill no., and Delete (everything is reversed; the number is not reused).
- **Numbers** are automatic and never reused, as in the old program. **Admin → Document numbers** sets the prefix and the next number for bills and purchase invoices, e.g. next sale `S-14727`, next purchase `P-12696`, to carry on from Apna Accountant.
- **Reports hub** (sidebar **Reports**) is laid out like the old Reports menu — *Accounts Reports*: Chart of Accounts, Account Ledger, Accounts Reconciliation, Cash Book (F9), Day Book, Journal Book, Trial Balance, Trial Balance Between Dates, Book Balances, Receivable And Payable, Receivable And Payable CityWise, Receivable, Payable, Profit And Loss, Profit and Loss Between Dates, Balance Sheet, Balance Sheet Between Dates, Vouchers Printing; *Inventory Reports*: Daily Gross Profit, Party Reports › (party-wise sale, purchase, outstanding), Product Reports › (daily sale, daily purchase, product-wise sale and purchase, rate list with last sale and purchase rate), Stock Reports › (stock in hand as at a date, stock ledger, godown-wise stock, stock value, low stock); *Pending Delivery*. Every report has date filters, **Print** in the classic style (shop name centred, title underlined, ruled table) and **CSV**. Reports showing the books or profit need finance access.
- **Keyboard**: F2 sale invoice, F3 receive payment, F4 expense, F6 receive stock, **F9 Cash Book** (inside a bill or purchase invoice F9 saves, as before). Cmd/Ctrl+K lists them.

**How a purchase invoice is posted** (see `src/utils/purchaseInvoices.ts`): the bill discount and the other charges are spread over the lines in proportion to their amounts, and each line is received into stock at its *landed* rate (rate × bill total ÷ gross). So the discount lowers the cost of the stock and the charges raise it, and cost of goods sold (and gross profit) use what the goods really cost. Each line posts Dr Inventory 1200 / Cr Payable 2000; the few paisa of rounding left over post as one supplier row to 5150 Purchase price differences, so the supplier is owed exactly the bill total; *Paid now* is a normal supplier payment (Dr Payable / Cr Cash 1000 or Bank 1010). Every entry balances, so the trial balance stays balanced.

**Screens built elsewhere**: the classic menu and the Reports hub open these screens by id as soon as they exist (listed in `ACTIVE_SCREENS`, `src/types.ts`): `vouchers`, `account-ledger`, `chart-of-accounts`, `bank-accounts`, `city-report` (see `PARTNER_SCREENS` in `src/utils/classicMenu.ts`). Until then they open the Accounts tabs (Journal, General ledger, Chart of accounts) or the Receivable And Payable report. Book Balances and the Bank Book list every asset account 1000–1099, so new bank accounts appear there by themselves.

**Database**: run `supabase/migrate_v25_invoices_reports.sql` once (after `setup.sql`): `invoices."memoNo"`, `"enteredAt"`, `delivery`; the `purchase_invoices` table (locked like the rest when the shop is locked); `settings."classicMenu"`. Without it everything still works on the device.
...
14. `migrate_v25_invoices_reports.sql` (once, **run it after `setup.sql`** — not folded into `setup.sql`) — memo no., "entered on" and delivery order on bills (`invoices."memoNo"`, `"enteredAt"`, `delivery`), the `purchase_invoices` table (RLS follows the shop's lock), and `settings."classicMenu"`.
...
- Re-running `supabase/setup.sql` after an update is safe: once the shop is locked it leaves the lock on. If an update adds new tables, run `supabase/lock.sql` again afterwards so they are protected too.
